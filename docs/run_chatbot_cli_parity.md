...
| Insights distillation — *running* it | `fastworkflow run --generate_insights` | **CLI-only** | Teacher/student comparison needs the interactive CLI loop |
| Insights distillation — *reviewing* it | reading the two markdown files | **In Studio** since `[DR37]` reversed `[R24]` (2026-08-28): run markers on the turn list, per-pass waterfalls, the aligned teacher/student diff, the insight ledger with verdicts, and the corpus aggregates | The rules are the raw material for planner skills and verifiers, so they have to be checkable against their evidence; reviewing is not running |
