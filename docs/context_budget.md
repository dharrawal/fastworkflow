...
`fastworkflow/context_budget.py`. Bead `ido-pyw.1`.
...
is given, how large one result page or one `search_memory` answer may be, how
much text each process-local cache may hold, and how much an offload must save
...
Before `ido-pyw.1` they were seven independent environment variables with seven
...
| 3 | the documented fallback | `REFERENCE_WINDOW_TOKENS` = **131,072**, the window every accepted result-search experiment ran at. |
...
`max_input_tokens = 131072` — reproduces the accepted stack's pinned values
exactly.
...
| `result_page_max_bytes` | one result-handle page observation, header included | 3/512 (≈ 0.586 %) | **3,072** | `FW_RESULT_PAGE_MAX_BYTES` |
...
| `result_handle_hot_max_bytes` | process-local hot cache of result-handle rows | 1/2 | **262,144** | `FW_RESULT_HANDLE_HOT_MAX_BYTES` |
| `offload_min_saving_bytes` | minimum UTF-8 bytes an offload must free | 1/512 (≈ 0.195 %) | **1,024** | `FW_OFFLOAD_MIN_SAVING_BYTES` |

The two hot caches take half the window each because they are not prompt: they
are what the prompt can be rebuilt from, and the durable copy is SQLite, so an
...
Unlike the pre-`ido-pyw.1` readers, an override written into a workflow's own
...
```json
{
  ...
  "budgets": {
    ...
    "result_page_max_bytes": 3072,
    "search_answer_max_bytes": 3072,
    "offload_hot_max_bytes": 262144,
    "result_handle_hot_max_bytes": 262144,
