...
- **Eager artifact validation** `[X3a]`: at the `invoke_command` return boundary, a cheap
  ...
  (unconditional since `ido-pyw.1`, which removed `FW_EAGER_ARTIFACT_VALIDATION`).
...
| ~~`FW_EAGER_ARTIFACT_VALIDATION`~~ | — | removed in `ido-pyw.1`; the dev-time validator `[X3a]` is unconditional |
