...
Moved out of ``run_fastapi_mcp/conversation_store.py`` unchanged when the
observability DB became the single source of truth for conversations. Only the
LLM call and its timeout plumbing live here; where the label is written, and
which topic wins a collision, are the store's business
(``ObservabilityStore.record_conversation_label``).

``conversation_store`` re-exports these names so its retired-but-not-deleted
tests keep exercising one implementation rather than a copy.
"""
