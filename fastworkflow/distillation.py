...
class DistillationSession:
    ...
    def __init__(self, wec: "fastworkflow.WorkflowExecutionContext"):
        ...
        # `_record_extraction` fills this; `_extract_close_attributes` reads it
        # back when the caller that opened the extract span closes it.
        self._extract_raw_output: dict[str, str] = {}
    ...
    def _extract_span_attributes(
        ...
        *,
        ...
        """The `fw.distill.extract` attributes for one extractor call (§8).

        Returns the attribute map rather than opening the span, so each caller
        opens and closes its own. The §12.0-delta-5 contract scanner learns
        which span a site emits from a direct `tracing.start_span` call, and
        resolves an attribute helper only when it returns a dict literal —
        this shape satisfies both, and puts the span's lifetime where a reader
        can see it bracket the extractor call.
    ...
    def _record_extraction(
        ...
        """Record the extraction outcome — empty or not (§13.3).
        ...
        The extract span is closed by the CALLER, which is also the function
        that opened it. That pairing is deliberate: the §12.0-delta-5 contract
        scanner recovers each emitter's key set by AST, and attributes a
        close-only `end_span` by falling back to the module's span names —
        which is ambiguous here, because this module emits four. Leaving the
        close beside the open makes the span this site writes unambiguous to a
        reader and to the scanner alike. The attributes are handed back through
        `_extract_close_attributes`.
        ...
        self._extract_raw_output[kind] = raw_output
    ...
    def _extract_close_attributes(self, kind: str) -> dict[str, Any]:
        """The `fw.distill.extract` closing attributes for one extraction.

        A dict literal in a named helper for the same reason
        `_extract_span_attributes` is one: the contract scanner resolves an
        attribute helper defined in the emitting module, and cannot see through
        an instance attribute.
        """
        outcome = self._extractions.get(kind)
        return {
            "raw_output": self._extract_raw_output.get(kind),
            "parsed_count": len(outcome.insights) if outcome else 0,
            "empty_reason": outcome.empty_reason if outcome else None,
            "insight_ids": list(outcome.insight_ids) if outcome else [],
        }
    ...
    def extract_insights(
        ...
        with self.chat_session.distillation_pass_scope("extractor"):
            span = tracing.start_span(
                ...
                attributes=self._extract_span_attributes(
            ...
            kept = self._record_extraction(
                span, INSIGHT_KIND_EXECUTION, raw_insights, insights
            )
            tracing.end_span(
                self.chat_session,
                span,
                attributes=self._extract_close_attributes(INSIGHT_KIND_EXECUTION),
            )
            return kept
    ...
    def extract_planning_insights(
        ...
        with self.chat_session.distillation_pass_scope("extractor"):
            ...
            tracing.end_span(
                self.chat_session,
                span,
                attributes=self._extract_close_attributes(INSIGHT_KIND_PLANNING),
            )
            return kept
