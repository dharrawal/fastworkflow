"""
Legacy per-channel conversation persistence — RETIRED from production paths.

Nothing under ``fastworkflow/`` imports this module any more. The observability
DB (``observability_store.py``) is the single source of truth for conversations,
turns, memory rebuild and feedback as of the Phase-7 consolidation
(``docs/observability_phase7_consolidation_design.md`` §2.8). Existing
``conversations/<channel_id>.sqlite3`` files are left on disk untouched:
readable by older builds, deletable by the operator, and erased along with the
channel by ``run_forget_channel`` (ruling C1).

The module and its tests survive the cutover by design — ruling C5 makes
physical deletion a separate change requiring explicit approval — so this stays
importable and behaviour-preserving. Do not wire it back into a production path.

Topic generation moved to ``fastworkflow.conversation_labeling`` (§2.5) and is
re-exported here so the tests that still drive it exercise one implementation.
"""
