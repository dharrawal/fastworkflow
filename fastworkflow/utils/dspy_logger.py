...
class DSPyObservabilityCallback(BaseCallback):
    ...
    def on_lm_start(self, call_id: str, instance: Any, inputs: dict[str, Any]):
        ...
        # `call_kwargs.timeout`), and against the nested shape each of those
        # lookups silently returned nothing. The attribute is flattened once,
        # here, so the recorded shape is the shape the readers ask for. A key
        # already present at the top level wins: it came from the callback
