...
class TestFlushIsADurabilityBarrier:
    ...
    def test_a_barrier_settles_under_continuous_concurrent_emission(self, db_path):
        ...
        try:
            ...
            time.sleep(0.1)
            # A GENEROUS timeout, deliberately. The property under test is
            # "settles rather than starving indefinitely", and under the old
            # quiescence rule this starves for however long the timeout is —
            # so a large one discriminates just as sharply while giving a
            # loaded machine room. `flush` reports a timeout as False, so the
            # returned bool already carries the liveness claim; asserting a
            # wall-clock bound on top of it only added flakiness under load.
            ok = sink.flush(timeout=60.0, trace_id="t-barrier")
        finally:
            stop.set()
            noise.join(10.0)
            sink.close()
